...
const findLogementDetails = (logementId: string) => {
    return tableDataFormation.find(entry => entry.noi === logementId);
};

export const Route = createFileRoute("/$enquete/formation/codeur/logement/$logement")({
    component: () => {
        // eslint-disable-next-line react-hooks/rules-of-hooks
        const match = useMatch("/$enquete/formation/codeur/logement/$logement");
        ...
        const handlePersonClick = (index: number) => {
            navigate({ to: `/enquete/formation/codeur/logement/${logement}/${index + 1}` });
        };
